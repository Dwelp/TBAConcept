...
public class GDEImportExcel : EditorWindow {
   private const string menuItemLocation = GDEManagerWindowBase.rootMenuLocation + "/Import Spreadsheet";
...
#pragma warning disable 0649
    ...
    [MenuItem(menuItemLocation, false, GDEManagerWindowBase.menuItemStartPriority+53)]
    ...
    void OnGUI()
    {
        ...
        if (rateBoxStyle == null)
        {
            rateBoxStyle = new GUIStyle(GUI.skin.box);
            rateBoxStyle.normal.background = (Texture2D)Resources.LoadAssetAtPath(GDEConstants.BorderTexturePath, typeof(Texture2D));
    ...
    void ProcessSheet()
    {
        try
        {
            ...
            foreach(string name in sheetNames)
            {
                ...
                List<FieldInfo> fields = GetFields(sheetRows);
    ...
    void CreateSchema(string name, List<FieldInfo> fields)
    {
        ...
        {
            if (field.type != BasicFieldType.Undefined)
            {
    ...
    void CreateItems(string schemaName, List<FieldInfo> fields, List<DataRow> sheetRows)
    {
        ...
        DataRow currentRow;
        for(int index=2;  index<sheetRows.Count;  index++)
        {
    ...
    List<FieldInfo> GetFields(List<DataRow> sheetRows)
    {
        List<FieldInfo> fields = new List<FieldInfo>();

        if (sheetRows == null || sheetRows.Count < 2)
            return new List<FieldInfo>();
        
        DataRow fieldNames = sheetRows[0];
        DataRow fieldTypes = sheetRows[1];
